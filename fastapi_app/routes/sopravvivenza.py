...
@router.get("/sopravvivenza-mensile")
def get_sopravvivenza_mensile(
    rows:          List[str] = Query(default=[]),
    cols:          List[str] = Query(default=[]),
    anno:          List[str] = Query(default=[]),
    sesso:         List[str] = Query(default=[]),
    ...
    """Calcola la sopravvivenza osservata a 5 anni con persi al follow-up per mese."""
    logger.info("GET /api/sopravvivenza/sopravvivenza-mensile rows=%s cols=%s", rows, cols)
    ...
    extra_where = (" AND " + " AND ".join(conditions)) if conditions else ""
    partition_sql = f"{partition_by_clause} " if partition_by_clause else ""
    final_group_by = f"GROUP BY {all_dims_sql}" if all_dims else ""
    final_order_by = f"ORDER BY {all_dims_sql}" if all_dims else ""

    estrazione = """
        WITH
        tempi AS (
            SELECT DISTINCT date_trunc('month', intervallostatoinvita) AS intervallo
            FROM scheda_caso
            WHERE intervallostatoinvita BETWEEN '0 year' AND '5 year'
        ),
        dati AS (
            SELECT
                {all_dims_select_sql}
                tempi.intervallo,
                count(CASE WHEN date_trunc('month', scheda_caso.intervallostatoinvita) = tempi.intervallo
                                AND scheda_paziente.statoinvita IN ('2') THEN true END) AS morti,
                count(CASE WHEN scheda_caso.intervallostatoinvita > tempi.intervallo
                                AND scheda_paziente.statoinvita IN ('2') THEN true
                           WHEN scheda_caso.intervallostatoinvita >= tempi.intervallo
            ...
            SELECT
                ...
                exp(
                    sum(CASE WHEN vivi > 0 AND morti > 0
                             THEN ln(1.0 * vivi / (vivi + morti))
                             ELSE 0 END)
                    OVER ({partition_sql}ORDER BY intervallo
                          ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
                ) AS sopravvivenza_cumulata
            FROM dati
        )
    ...
    try:
        with get_db() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(estrazione)
                results = [dict(r) for r in cur.fetchall()]
                logger.info(f"Retrieved {len(results)} monthly sopravvivenza records")
                return {"count": len(results), "data": results}
    except Exception as exc:
        logger.error(f"Error fetching monthly sopravvivenza: {exc}")
        raise HTTPException(status_code=500, detail=str(exc))
